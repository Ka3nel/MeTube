    ...
    const promiseOfSomeData = fetch("http://localhost:8080/mediaFiles")
                .then(response => response.json())
                .then(data => {return data});

window.onload = async () => {
    mediaSource = await promiseOfSomeData;
    ...
    function drawAutoPlayOn() {
        ...
        autoPlay.src = "./icons/switch-on.png";
        var size = 20;
        marginBottom = 10;
        marginRight = 50;
    ...
    function drawAutoPlayOff() {
        ...
        var autoPlay = document.createElement("img");
        autoPlay.src = "./icons/switch-off.png";
        var size = 20;
        marginBottom = 10;
        marginRight = 50;
        ctx.drawImage(autoPlay, canvas.width - marginRight, canvas.height - marginBottom / 2 - 1.6 * size, 1.85 * size, 2 * size);
    ...
    submit_btn.addEventListener("click", () => {
        mediaSource.push("media/" + file.files[0].name);
        desenarePlaylist();
    });
